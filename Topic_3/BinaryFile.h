...
public:
	...
	int write(vector<Point>& points);					//Method that saves a vector of points to a binary file, on success the method returns "Success" with the value 0
	...
	int read(vector<Point>& points);					//Method reads a vector of points from a binary file, on success the method returns "Success" with value 0
														//In case of an attempt to read from a closed file, the method will return an error "FileClosed" with the value 2
														//In case of an error, the method will return the error "Error" with a value of 3

	int read(Point& point, int i);						//Method reads the given point from the binary file, on success the method returns "Success" with the value 0
