...
{
public:
	CsvFIle(std::string path, std::string openMode);					//Constructor opens the file from the given path
	...
	virtual int write(std::vector<Point>& points);						//Method that saves a vector of points to a CSV file, on success the method returns "Success" with the value 0
	...
	virtual int read(std::vector<Point>& points);						//Method reads a vector of points from a CSV file, on success the method returns "Success" with value 0
																		//In case of an attempt to read from a closed file, the method will return an error "FileClosed" with the value 2
																		//In case of an error, the method will return the error "Error" with a value of 3

	virtual int read(Point& point , int i);								//Method reads the given point from the CSV file, on success the method returns "Success" with the value 0
